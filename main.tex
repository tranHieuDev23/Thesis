...
PatchGAN là mô hình discriminator được để xuất trong nghiên cứu Pix2pix của các nhà khoa học thuộc trường Đại học California. PatchGAN sử dụng mạng convolutional để phân loại ảnh theo từng mảng (patch) nhỏ. Kết quả trung bình của tất cả các mảng sẽ được sử dụng làm kết quả phân loại cuối cùng. Vì làm việc vùng ảnh nhỏ, PatchGAN nhạy cảm với các tiểu tiết bị lỗi trên ảnh hơn các mô hình discriminator cơ bản.

\section{Kiến trúc cụ thể}
